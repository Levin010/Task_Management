...
const getDashboardDescription = () => {
  if (user.value?.role === 'admin') {
    return 'Admin overview of the task management system'
  } else if (user.value?.role === 'manager') {
    return 'Overview of tasks you have created'
  } else if (user.value?.role === 'member') {
    return 'Overview of your assigned tasks'
  }
  return 'Overview of your task management system'
}

const getCardCount = () => {
  if (user.value?.role === 'admin') {
    return 6 // Total Users + Total Tasks + Pending + In Progress + Overdue + Completed
  } else if (user.value?.role === 'manager') {
    return 5 // Tasks Created + Pending + In Progress + Overdue + Completed
  } else {
    return 5 // Total Tasks + Pending + In Progress + Overdue + Completed
  }
}
    ...
    <div class="flex-1 flex flex-col min-w-0">
      ...
      <main class="flex-1 p-4 sm:p-6 overflow-y-auto">
        ...
        <div v-if="isLoading" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
          <div v-for="i in getCardCount()" :key="i" class="bg-white p-4 sm:p-6 rounded-lg shadow-sm border border-gray-200 animate-pulse">
        ...
        <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
          ...
          <!-- Admin and Manager: Total Tasks (all or created by manager) -->
          <div v-if="user?.role === 'admin' || user?.role === 'manager'" class="bg-white p-4 sm:p-6 rounded-lg shadow-sm border border-gray-200">
            <div class="flex items-center justify-between">
              <div>
                <p class="text-xs sm:text-sm font-medium text-gray-600">
                  {{ user?.role === 'admin' ? 'Total Tasks' : 'Tasks Created' }}
                </p>
          ...
          <!-- Members Only: Total Tasks (assigned to them) -->
          <div v-if="user?.role === 'member'" class="bg-white p-4 sm:p-6 rounded-lg shadow-sm border border-gray-200">
            <div class="flex items-center justify-between">
              <div>
